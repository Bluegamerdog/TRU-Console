...
export default class OperatorVerifySubsommand implements ICommand {
    name: Lowercase<string> = "verify";
    description: string = "Allows users to verify with TRU Console.";
    guardsman: Guardsman;
    ...
    async execute(interaction: ChatInputCommandInteraction<"cached">): Promise<void> {
        ...
        if (existingUserData) {
            const continueVerification = await new Promise<boolean>(async () => {
                    ...
                    new ActionRowBuilder<ButtonBuilder>()
                        .addComponents(
                            new ButtonBuilder()
                                ...
                                .setStyle(ButtonStyle.Link)
                                .setURL(`https://www.roblox.com/groups/15155175/QSO-Tactical-Response-Unit`),
                            new ButtonBuilder() // Add a new button for confirming join request
            ...
            const collector = channel?.createMessageComponentCollector({
                time: 300000, // 5 minutes in milliseconds (temp 30 sec)

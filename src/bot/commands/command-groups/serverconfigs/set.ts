...
export default class ConfigsSetSubcommand implements ICommand {
    ...
    async execute(interaction: ChatInputCommandInteraction<"cached">): Promise<void> {
        ...
        let embed = new EmbedBuilder()
            ...
            .setColor(0x0099FF)
            .addFields(
                { name: 'Response Ping role:', value: `<@&${responsePingRoleID}>` },
                { name: 'Response Announce channel:', value: responseAnnounceChannelID ? `<#${responseAnnounceChannelID}>` : "Not set" },
                { name: 'Member role:', value: `<@&${memberRoleID}>` },
                { name: 'Host role:', value: `<@&${hostRoleID}>` },
                { name: 'Command role:', value: `<@&${commandRoleID}>` },
                { name: 'Developer role:', value: `<@&${developerRoleID}>` }
            )

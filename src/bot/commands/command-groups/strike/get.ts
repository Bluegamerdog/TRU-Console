...
import {
    ...
    ChatInputCommandInteraction, Colors, Embed, EmbedBuilder,
...
export default class StrikeGetSubcommand implements ICommand {
    name: Lowercase<string> = "get";
    description: string = "Gets a specific or list of strikes.";
    guardsman: Guardsman;
            ...
            if (strikeData.length > 0) {
                for (const strike of strikeData) {
                    responseEmbed.addFields(
                        { name: `ID: ${strike.id} | <t:${strike.timestamp}>`, value: `Reason: ${strike.reason}` }
                    )
                }
            }

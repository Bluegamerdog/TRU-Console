...
export default class qbGetSubcommand implements ICommand {
     name: Lowercase<string> = "fetch";
     description: string = "Gets a specified or the current quota blocks.";
     guardsman: Guardsman;
     ...
     async execute(interaction: ChatInputCommandInteraction<"cached">): Promise<void> {
          ...
          else { quotaBlockData = await this.guardsman.database.quotaBlocks.findFirst({ where: { blockActive: true } }) }
          ...
          if (quotaBlockData.blockActive) { blockStatus = "Active" }

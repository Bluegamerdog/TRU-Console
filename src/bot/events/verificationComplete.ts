...
export default async (guardsman: Guardsman, discordId: string) => {
    try {
        ...
        const reply = await interaction.editReply({
            ...
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        "Discord account verification was successful! Please run </operator update:1143537021487427807> to obtain roles."
                    )
        ...
        const logChannel = guardsman.bot.channels.cache.get("1143525855960760390") as TextChannel;
     
        if (logChannel) {
            await logChannel.send({
                content: `<@776226471575683082> | <@${interaction.member.id}> has registered with TRU Console!`
            });
        } else {console.log("Channel not found while trying to send verification notice!")}

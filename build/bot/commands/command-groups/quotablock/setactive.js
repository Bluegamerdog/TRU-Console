...
export default class qbSetactiveSubcommand {
    name = "setactive";
    description = "Sets a quota block as active.";
    guardsman;
    options = [
    ...
    async execute(interaction) {
        ...
        await this.guardsman.database.quotaBlocks.updateMany({ where: { blockActive: true }, data: { blockActive: false } });
        const newQuotaBlock = await this.guardsman.database.quotaBlocks.update({ where: { blockNum: newBlockNum }, data: { blockActive: true } });
        if (!newQuotaBlock) {
            await interaction.reply({
                embeds: [
                    new EmbedBuilder()
